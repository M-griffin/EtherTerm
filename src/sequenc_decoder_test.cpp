...
/**
 * @brief Oblivion/2 XRM Unit Tests for SequenceDecoder.
 * @return
 */
...
SUITE(XRMSequenceDecoder)
{

    // Test Cases if for debugging issues with Decoder.
    // Debugging issue on + in ESC[0+ is being dropped.
    TEST(NumberCurotffAfterESCParams)
    {
        
        std::cout << "\r\n======================================= \r\n" << std::endl;
        
        // Setup Mock Sesstion which has queue pasted back to it.
        session_ptr session(new Session());
        
        // Queue to loop after the decoder.
        // session->m_data_queue
        sequence_decoder_ptr seq(new SequenceDecoder(session));
        
        // Sequence seen in VTTEST
        std::string esc_sequence = "\x1b[1D\x1b[1C\x1b[0+\x1b[2Dtest";
        seq->decodeEscSequenceData(esc_sequence);
        
        std::cout << "ESC SEQ: " << esc_sequence << std::endl;
        while (!session->m_data_queue.is_empty()) {
            MessageQueue msgQueue = std::move(session->m_data_queue.dequeue());
            ...
            if(msgQueue.m_text.empty()) 
            {
                ...
                {
                    for (int i = 0; i < msgQueue.m_queueParams.size(); i++)
                    {
                        std::cout << static_cast<char>(msgQueue.m_queueParams[i]) << " - " << msgQueue.m_queueParams[i] << std::endl;
                    }
                }                
            }
            else 
            {
                std::cout << "msgQueue.m_text: " << msgQueue.m_text << std::endl;
            }
        }
